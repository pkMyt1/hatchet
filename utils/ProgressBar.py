...
class ProgressBar:
    ...
    def __init__(self, total, length, counter=0, verbose=False, decimals=1, fill=chr(9608), lock=None,
    ...
    def progressLock(self, advance=True, msg=""):
        ...
        result = '%s%s%s |%s| %s%s%% %s%s' % (sp.bcolors.BBLUE, self.prefix, sp.bcolors.ENDC, bar, sp.bcolors.BBLUE,
                                              percent, self.suffix, sp.bcolors.ENDC)

        # result = '{}{}{} |{}| {}{}% {}{}\n'\
        #     .format(sp.bcolors.BBLUE, self.prefix, sp.bcolors.ENDC, bar, sp.bcolors.BBLUE, percent, self.suffix,
        #             sp.bcolors.ENDC)

        msg = '[{:%Y-%b-%d %H:%M:%S}]'.format(datetime.datetime.now()) + msg
        if not self.verbose:
            # toprint = rewind + result + " %s[%s]%s" % (sp.bcolors.BBLUE, msg, sp.bcolors.ENDC)
            toprint = "{}{} {}[{}]{}".format(rewind, result, sp.bcolors.BBLUE, msg, sp.bcolors.ENDC)
        else:
            # toprint = rewind + sp.bcolors.BBLUE + msg + sp.bcolors.ENDC + "\n" + result
            toprint = "{}{}{}{}\n{}".format(rewind, sp.bcolors.BBLUE, msg, sp.bcolors.ENDC, result)
        with self.lock:
            write(toprint)
            flush()
    ...
    def progressNoLock(self, advance=True, msg=""):
        ...
        result = '{} |{}| {}% {}\n'.format(self.prefix, bar, percent, self.suffix)
        if not self.verbose:
